...
async def test_balanced_scraper(num_restaurants=20):
    """Test the Priority 1 Balanced Scraper"""
    print("🚀 Starting Priority 1 Balanced Scraper Test")
    print("=" * 50)
    ...
    # Initialize scraper
    scraper = Priority1BalancedScraper(headless=True, timeout=30000)
        ...
        for i, restaurant in enumerate(test_restaurants, 1):
            ...
            if result['scraping_success']:
                ...
                if items_with_prices:
                    ...
                    sample_items = result['items'][:2]
                    for item in sample_items:
                        print(f"   - {item['name']} (no price)")
            else:
    ...
    # Save results
    output_file = f'priority1_balanced_test_results_n{num_restaurants}.json'
...
async def save_results(results, filename):
    ...
    try:
        ...
        summary = {
            'test_info': {
                'scraper_version': 'Priority1BalancedScraper',
...
def print_summary(results, total_time):
    """Print test summary"""
    print("\n" + "=" * 60)
    print("📊 PRIORITY 1 BALANCED SCRAPER TEST SUMMARY")
    print("=" * 60)
    ...
    print("\n📈 COMPARISON WITH PREVIOUS SCRAPERS:")
    print("Enhanced Dynamic Scraper: 35% success rate, 0% price coverage")
    print("Priority 1 Improved Scraper: 0% success rate (too strict filtering)")
    print(f"Priority 1 Balanced Scraper: {success_rate:.1f}% success rate, {(items_with_price / total_items * 100) if total_items > 0 else 0:.1f}% price coverage")
    
    if success_rate >= 50:
        print("🎉 TARGET ACHIEVED: 50%+ success rate!")
...
if __name__ == "__main__":
    asyncio.run(test_balanced_scraper(20))